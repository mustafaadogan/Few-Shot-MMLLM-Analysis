...
class LLaVA:
    ...
    def create_descriptions(self, data):
        """
        Create CoT descriptions using the LLaVA model on the given data.
        ...
        for item in tqdm(data):
            ...
            caption_prompt = f"<|im_start|>system\n{item['query_prompt']}<|im_end|><|im_start|>user\n<image>\n{item['query_prompt']} Sentence: {item['query_raw_texts'][0]}<|im_end|><|im_start|>assistant\n"
            foil_prompt = f"<|im_start|>system\n{item['query_prompt']}<|im_end|><|im_start|>user\n<image>\n{item['query_prompt']} Sentence: {item['query_raw_texts'][1]}<|im_end|><|im_start|>assistant\n"
...
model_registry.register_model("llava", (llava_instance.load_model, llava_instance.create_descriptions, llava_instance.prepare_results))
