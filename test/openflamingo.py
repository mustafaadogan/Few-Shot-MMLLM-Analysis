...
class OpenFlamingo:
    ...
    def test(self, data):
        """
        Test the OpenFlamingo model on the given data.
            ...
            for indexes in index_list:
                ...
                for raw_texts, cot_info in zip(support_class_text_list, cot_info_list):
                    ...
                    if is_caption_example:
                        prompt += f"<image>{item['support_prompt']} {support_caption} {cot_caption}<|endofchunk|>"
                    else:
                        prompt += f"<image>{item['support_prompt']} {support_foil} {cot_foil}<|endofchunk|>"


                query_caption, query_foil, is_caption_query = item['query_raw_texts'][0], item['query_raw_texts'][1], item['query_raw_texts'][2]

                if is_caption_query:
                    prompt += f"<image>{item['query_prompt']} {query_caption} Answer:"
                else:
                    prompt += f"<image>{item['query_prompt']} {query_foil} Answer:"
